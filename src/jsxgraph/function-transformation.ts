...
const TransformationSchema = z.object({
  ...
  parameters: z.object({
    ...
    axis: z.enum(["x", "y", "both"]).optional().describe("Reflection axis (for reflect)"),
    innerFunction: z.string().optional().describe("Inner function g(x) for composition"),
  }).optional().describe("Parameters for the transformation"),
...
const schema = {
  baseFunction: z.object({
    expression: z.string().describe("Base function expression, e.g., 'x^2', 'Math.sin(x)', 'Math.sqrt(x)'"),
    color: z.string().optional().default("#0066cc").describe("Color of the base function"),
  ...
  highlightPoints: z
    .array(z.object({
      ...
      showTransformed: z.boolean().optional().default(true).describe("Show corresponding points on transformed functions"),
    }))
    .optional()
