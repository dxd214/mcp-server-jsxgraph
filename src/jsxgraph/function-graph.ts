...
const FunctionDataSchema = z.object({
  expression: z
    .string()
    .describe(
      "Mathematical function expression, e.g., 'Math.sin(x)', 'x^2 + 2*x - 1', 'Math.exp(x)'",
    ),
  color: z
    .string()
    .optional()
  ...
  strokeWidth: z
    ...
    .optional()
    .default(2)
    .describe("Width of the function curve"),
  ...
  domain: z
    ...
    .optional()
    .describe(
      "Domain of the function [min, max], if not specified, uses boundingBox x range",
    ),
  dash: z
    .number()
    .optional()
...
const schema = {
  functions: z
    ...
    .describe(
      "Array of mathematical functions to plot. Each function has an expression and optional styling.",
    )
  ...
  integralBounds: z
    ...
    .optional()
    .describe(
      "Bounds for integral area [a, b], required if showIntegral is true",
    ),
  tangentAt: z
    .number()
    .optional()
