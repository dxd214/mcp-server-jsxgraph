...
const StepSchema = z.object({
  ...
  elements: z.array(z.object({
    type: z.enum(['point', 'line', 'curve', 'polygon', 'arrow', 'text']).describe('Element type'),
    ...
    style: z.object({
      ...
      dash: z.number().optional().describe('0: solid, 1: dotted, 2: dashed'),
      opacity: z.number().optional(),
      fillColor: z.string().optional(),
      fillOpacity: z.number().optional(),
      visible: z.boolean().optional()
    }).optional().describe('Style configuration')
  ...
  })).optional().describe('Text annotations'),
  shadingAreas: z.array(z.object({
    type: z.enum(['between-curves', 'polygon', 'region']),
    bounds: z.any().optional(),
    color: z.string().optional(),
    opacity: z.number().optional()
  })).optional().describe('Shading areas'),
  ...
  })).optional().describe('Pointer arrows')
});

// Input schema for polynomial steps
const schema = {
  polynomial: z.object({
    expression: z.string().describe('Polynomial expression, e.g., "-2*(x-3)^2*(x^2-16)"'),
    expandedForm: z.string().optional().describe('Expanded form for calculations'),
    zeros: z.array(z.object({
      x: z.number(),
    ...
    })).optional().describe('Critical points'),
  ...
  steps: z.array(StepSchema).optional().describe('Custom steps (if not provided, will generate default steps)'),
  
  title: z.string().default('Polynomial Function Step-by-Step Analysis').describe('Chart title'),
  ...
  playSpeed: z.number().default(3000).describe('Auto-play speed in milliseconds'),
  
  outputFormat: z.enum(['javascript', 'html', 'image']).default('javascript').describe('Output format'),
